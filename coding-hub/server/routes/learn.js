...
router.get('/overview', wrap(async (req, res) => {
  ...
  res.json({
    name: getSetting('site_name', 'Coding Hub'),
...
router.post('/challenges/:id/complete', wrap(async (req, res) => {
  ...
  if (!already) {
    ...
    grantBadge(req.user.id, 'first_challenge');
  }
...
router.get('/bookmarks', wrap(async (req, res) => {
  const rows = all('SELECT * FROM bookmarks WHERE user_id = ? ORDER BY created_at DESC', req.user.id);
  const bookmarks = [];
...
router.post('/bookmarks', wrap(async (req, res) => {
  ...
  const existing = get('SELECT 1 AS x FROM bookmarks WHERE user_id = ? AND kind = ? AND ref_id = ?',
    req.user.id, kind, refId);
  if (existing) {
    run('DELETE FROM bookmarks WHERE user_id = ? AND kind = ? AND ref_id = ?', req.user.id, kind, refId);
    return res.json({ bookmarked: false });
  }
  run('INSERT INTO bookmarks (user_id, kind, ref_id) VALUES (?, ?, ?)', req.user.id, kind, refId);
