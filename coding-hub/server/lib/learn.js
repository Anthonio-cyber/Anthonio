...
import { awardXp, grantBadge } from './xp.js';
...
export function isBookmarked(userId, kind, refId) {
  return !!get('SELECT 1 AS x FROM bookmarks WHERE user_id = ? AND kind = ? AND ref_id = ?', userId, kind, refId);
}
...
export function checkQuestionBadges(userId) {
  ...
  if (answered >= 1000) grantBadge(userId, 'questions_1000');
  if (answered >= 500) grantBadge(userId, 'questions_500');
  if (answered >= 100) grantBadge(userId, 'questions_100');
  if (answerStreak(userId) >= 25) grantBadge(userId, 'sharp_shooter');
}
...
export function checkSubjectMastery(userId, subjectId) {
  ...
  if (!get('SELECT 1 AS x FROM badges WHERE key = ?', key)) return;
  ...
  if (totals.total > 0 && totals.done >= totals.total) grantBadge(userId, key);
}
...
export function completeLesson(userId, lesson) {
  ...
  grantBadge(userId, 'first_lesson');
