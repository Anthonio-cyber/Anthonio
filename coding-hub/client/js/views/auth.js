      ...
      <section class="auth-side">
        <div class="auth-side-inner">
          <div class="brand" style="padding:0 0 2rem">
            <span class="brand-mark" style="width:48px;height:48px;border-radius:14px;font-size:1.1rem">&lt;/&gt;</span>
          ...
          <p class="auth-sub">${esc(store.config.welcomeMessage || 'Lessons, coding challenges and thousands of practice questions - with progress tracking and private messaging.')}</p>
          <ul class="auth-points">
            <li>${icon('message', 18)}<span>Message other learners and club groups</span></li>
...
function showForm(mode, onSignedIn) {
  ...
  if (mode === 'register') {
    ...
    card.innerHTML = `
      <h2>Welcome back</h2>
      <p class="muted small">Sign in to keep learning.</p>
      <form id="auth-form" novalidate>
        <div class="field">
  ...
  card.querySelector('#auth-form').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      setUser(result.user);
      toast(mode === 'register' ? 'Welcome to the Coding Hub.' : `Welcome back, ${result.user.displayName}.`, 'success');
      await onSignedIn();
    } catch (err) {
