...
export default async function practiceView({ mount, query }) {
  ...
  async function submit(questionId, answer) {
    ...
    try {
      ...
      // Finishing a whole set with everything right is worth a fuss.
      const answered = results.size;
      const correct = [...results.values()].filter((r) => r.correct).length;
      if (answered === questions.length && questions.length >= 5 && correct === answered) {
        celebrate({
          title: 'Perfect set!',
          subtitle: `${correct} out of ${answered}, no mistakes`,
          xp: [...results.values()].reduce((sum, r) => sum + (r.pointsAwarded || 0), 0)
        });
      } else if (result.correct && result.stats.currentStreak > 0 && result.stats.currentStreak % 10 === 0) {
        celebrate({
          title: `${result.stats.currentStreak} in a row!`,
          subtitle: 'Every answer correct.',
          duration: 2800
        });
      }
    } catch (err) {
