...
function createMember({ username, password, displayName, email, roleKey = 'student' }) {
...
router.get('/config', (_req, res) => {
  res.json({
    ...
    className: getSetting('class_name', 'Grade 8'),
...
router.post('/register', loginLimiter, wrap(async (req, res) => {
  ...
  if (mode === 'closed') {
    throw new HttpError(403, 'The Grade 8 Hub is not accepting new members right now.', 'registration_closed');
  }
  ...
  if (mode === 'invite') {
    if (!code) throw new HttpError(400, 'An invitation code is required to join this class network.', 'code_required');
    ...
    roleKey = invitation.role_key || 'student';
  }
  ...
  if (invitation) {
    ...
    notify(invitation.created_by, {
      kind: 'invite_used',
      title: `${displayName} joined the hub`,
...
/** Lets the join screen check a code before the student fills the whole form. */
