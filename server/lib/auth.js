...
export const COOKIE = 'g8h_session';
...
export function loadUser(id) {
  const user = get(`
    SELECT u.*, p.avatar_url, p.cover_url, p.bio, p.theme, p.accent, p.streak_days, p.streak_date,
           p.show_online, p.class_section, p.favourite_subject, r.name AS role_name, r.rank AS role_rank
    FROM users u
...
/** Blocks the request unless a Grade 8 member is signed in. */
export function requireAuth(req, _res, next) {
  ...
  if (!req.user) return next(new HttpError(401, 'Please sign in to use the Grade 8 Hub.', 'unauthenticated'));
  return next();
}
