...
export function roleBadge(user) {
  ...
  const tone = { super_admin: 'danger', admin: 'danger', teacher: 'info', moderator: 'warning', club_admin: 'accent' }[user.role] || '';
...
// ---------------------------------------------------------------------------
// Choose a classmate (used for sharing, game invites and club invites)
// ---------------------------------------------------------------------------
export function openUserPicker({ title = 'Choose a classmate', confirmText = 'Select', exclude = [], onPick }) {
  const dialog = modal({
    title,
    body: `
      <div class="field">
        <input class="input" data-search placeholder="Search classmates by name..." autocomplete="off">
      </div>
      ...
      const load = async (term = '') => {
        try {
          ...
          results.innerHTML = list.length
            ...
            : emptyState({ iconName: 'search', title: 'No classmates found' });
        } catch (err) {
